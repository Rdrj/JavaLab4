...
public class StackSort {
    ...
    //declared two stacks two hold values
    VectorStack<Integer> lowervalues=new VectorStack();
    VectorStack<Integer> uppervalues=new VectorStack();
    //this loop will extract element from given array
    for(int i=0;i<data.length;i++)
    {
    	//if lowervalues stack is empty then simply push the element
    	if(lowervalues.isEmpty())
    	{
    		lowervalues.push(data[i]);
    	}
    	//if the element is greater than the top element of lowervalues stack then check further condition
    	else if(data[i]>=lowervalues.peek())
    	{
    		//if element is greater than uppervalue stack then pop from it and push into lowervalue one
    			while(!uppervalues.isEmpty() && (data[i]>uppervalues.peek()) )
    			{
    				int x=uppervalues.pop();
    				lowervalues.push(x);
    			}
    		//otherwise push in lowervalue
    		lowervalues.push(data[i]);
    	}
    	//if element is less than top value of lowervalue then check for further conditon
    	else if(data[i]<=lowervalues.peek())
    	{
    		//if element is less than lowervalue stack than pop from it and push into uppervalue one 
    		while(!lowervalues.isEmpty() && data[i]<lowervalues.peek())
    		{
    			int x=lowervalues.pop();
    			uppervalues.push(x);
    		}
    		//otherwise push in lowervalue simply
    		lowervalues.push(data[i]);
    	}	
    }
    //at the end push all the left elements in uppervalue to lowervalue 
    ...
    int n=data.length-1;
    //this loop pop value from lowervalue stack and insert into result array from end
    for(int i=0;i<data.length;i++)
    {
    	int x;
    	if(!lowervalues.isEmpty())
